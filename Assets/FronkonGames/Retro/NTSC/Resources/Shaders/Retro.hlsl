...
inline float Atanh(float y)
{
  return log((1.0 + y) / (1.0 - y)) / 2.0;
}

inline float WindowCosine(float x, float windowBiasParam)
{
  x = Atanh(x);
  x += windowBiasParam;
  x = tanh(x);
...
inline float RandomFloat01(inout uint state)
{
