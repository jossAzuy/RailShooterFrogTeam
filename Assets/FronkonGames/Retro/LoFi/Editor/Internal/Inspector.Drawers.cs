...
namespace FronkonGames.Retro.LoFi.Editor
{
  ...
  public abstract partial class Inspector : VolumeComponentEditor
  {
    ...
    protected void DrawEnumDropdownWithReset<T>(string name, string label = null, T defaultValue = default) where T : Enum
    {
      ...
      // Validate defaultValue - if 0 is not defined in the enum and defaultValue is 0, use first valid value
      T effectiveDefaultValue = defaultValue;
      int defaultIntValue = Convert.ToInt32(defaultValue);
      if (!Enum.IsDefined(typeof(T), defaultIntValue))
      {
        Array enumValues = Enum.GetValues(typeof(T));
        if (enumValues.Length > 0)
          effectiveDefaultValue = (T)enumValues.GetValue(0);
      }
      ...
      EditorGUILayout.BeginHorizontal();
      {
        ...
        EditorGUILayout.BeginHorizontal();
        {
          ...
          // Convert int to enum using the actual value, not as array index
          T currentValue;
          try
          {
            currentValue = (T)Enum.ToObject(typeof(T), currentInt);

            // Validate that the enum value is defined
            if (!Enum.IsDefined(typeof(T), currentInt))
            {
              Array enumValues = Enum.GetValues(typeof(T));
              currentValue = (T)enumValues.GetValue(0);
            }
          }
          catch
          {
          ...
          // Reset button
          bool isDefault = EqualityComparer<T>.Default.Equals(newValue, effectiveDefaultValue);
          if (ResetButton(effectiveDefaultValue, !isDefault) == true)
            parameter.value.intValue = Convert.ToInt32(effectiveDefaultValue);
        }
        EditorGUILayout.EndHorizontal();
