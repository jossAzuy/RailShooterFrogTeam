...
namespace FronkonGames.Retro.Handheld8Bit
{
  ...
  /// <summary> Vector2 with reset attribute. </summary>
  [AttributeUsage(AttributeTargets.Field)]
  public class Vector2SliderWithResetAttribute : PropertyAttribute
  {
    public readonly Vector2 value;
    public readonly float min;
    public readonly float max;
    ...
    public Vector2SliderWithResetAttribute(float[] value, float min, float max, string tooltip = "")
    {
      this.value = new Vector2(value[0], value[1]);
      this.min = min;
      this.max = max;
      this.tooltip = tooltip;
    }
