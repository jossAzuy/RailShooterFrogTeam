...
namespace FronkonGames.Retro.Pixelator.Editor
{
  ...
  public abstract partial class Inspector : VolumeComponentEditor
  {
    ...
    /// <summary> Draws a Vector3WithResetAttribute with sliders and reset using attribute configuration. </summary>
    ...
    /// <summary> Draws a Vector3Int field with reset. </summary>
    protected void DrawVector3IntWithReset(string name, string label = null, Vector3Int defaultValue = default)
    {
      SerializedDataParameter parameter = UnpackParameter(name);
      if (parameter == null)
        return;
      ...
      EditorGUI.BeginDisabledGroup(!isOverridden);
      Vector3Int value = parameter.value.vector3IntValue;
      EditorGUI.BeginChangeCheck();
      ...
      value = EditorGUI.Vector3IntField(valueRect, GUIContent.none, value);

      if (EditorGUI.EndChangeCheck() == true)
        parameter.value.vector3IntValue = value;
      EditorGUI.EndDisabledGroup();
      ...
      if (ResetButton(resetRect, defaultValue, value != defaultValue) == true)
        parameter.value.vector3IntValue = defaultValue;
      EditorGUI.EndDisabledGroup();
    }

    /// <summary> Draws a Gradient field with reset. </summary>
    protected void DrawGradientWithReset(string name, string label = null)
    {
      SerializedDataParameter parameter = UnpackParameter(name);
      if (parameter == null)
        return;
      ...
      float buttonsWidth = 16.0f + 19.0f + 19.0f + 4.0f;
      Rect fieldRect = new(rect.x, rect.y, rect.width - buttonsWidth, rect.height);
      Rect overrideRect = new(rect.x + rect.width - buttonsWidth + 2.0f, rect.y, 16.0f, rect.height);
      Rect searchRect = new(overrideRect.xMax + 1.0f, rect.y, 19.0f, rect.height);
      Rect resetRect = new(searchRect.xMax, rect.y, 19.0f, rect.height);
      ...
      EditorGUI.BeginDisabledGroup(!isOverridden);
      EditorGUI.BeginChangeCheck();
      EditorGUI.PropertyField(fieldRect, parameter.value, displayLabel);
      if (EditorGUI.EndChangeCheck() == true)
      {
        if (!isOverridden)
          parameter.overrideState.boolValue = true;
      ...
      EditorGUI.BeginDisabledGroup(!isOverridden);
      if (GUI.Button(searchRect, EditorGUIUtility.IconContent("d_Search Icon"), EditorStyles.miniLabel))
        PaletteTool.ShowTool(target as PixelatorVolume);

      if (GUI.Button(resetRect, EditorGUIUtility.IconContent("Refresh"), EditorStyles.miniLabel))
        GUI.changed = true;
      EditorGUI.EndDisabledGroup();
    }
