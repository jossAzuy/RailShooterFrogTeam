      ...
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          if (scriptableRendererData != null)
          {
            ...
            {
              if (scriptableRendererData.rendererFeatures[i] is Pixelator)
                return scriptableRendererData.rendererFeatures[i] as Pixelator;
            }
          }
        }
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Pixelator)
                  effects.Add(feature as Pixelator);
          }
