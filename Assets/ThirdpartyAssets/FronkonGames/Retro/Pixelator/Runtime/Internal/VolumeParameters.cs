...
namespace FronkonGames.Retro.Pixelator
{
  ...
  /// <summary> Clamped float parameter with no interpolation. </summary>
  [Serializable]
  public sealed class ClampedFloatParameterNoInterpolation : VolumeParameter<float>
  {
    public float min;
    public float max;

    public ClampedFloatParameterNoInterpolation(float value, float min, float max, bool overrideState = false)
      : base(value, overrideState)
    {
      this.min = min;
      this.max = max;
  ...
  /// <summary> Clamped int parameter with no interpolation. </summary>
  [Serializable]
